...
describe("runner log command parsing", () => {
  it("unwraps shell launch commands", () => {
    expect(formatShellCommandForDisplay('bash -lc "npm test"')).toBe(
      "npm test",
    );
  });
  ...
  it("recognizes help requests and names script resources", () => {
    expect(
      parseRunnerHelpCommandDetails("python /workspace/task_manager.py --help"),
    ).toEqual({
  ...
  it("tokenizes quoted shell arguments", () => {
    ...
    expect(extractQuotedArgument('--query "hello world"', "--query")).toBe(
      "hello world",
    );
  });

  it("detects source languages and escapes highlighted shell output", () => {
    expect(detectCodeLanguage("const value = 1;", "index.ts")).toBe(
      "typescript",
    );
    expect(renderShellTokenizedHtml("echo '<unsafe>'")).toContain(
      "&lt;unsafe&gt;",
    );
  });
});
