...
function renderIcon(icon: PlatformDataTableIcon | undefined, className: string, size = 15): ReactNode {
  if (!icon) return null;
...
function toSortingState(value: PlatformDataTableSortState | null): SortingState {
...
function normalizePaginationState(
  ...
  const pageSize = Math.max(1, Math.floor(Number(value?.pageSize) || DEFAULT_PAGE_SIZE));
  return { pageIndex, pageSize };
}

function getFloatingPosition(anchor: FloatingAnchor, width: number, height: number): { left: number; top: number } {
  const viewportWidth = typeof window === "undefined" ? width + VIEWPORT_GUTTER * 2 : window.innerWidth;
  const viewportHeight = typeof window === "undefined" ? height + VIEWPORT_GUTTER * 2 : window.innerHeight;
  ...
  const top = preferredTop + height + VIEWPORT_GUTTER <= viewportHeight
    ? preferredTop
...
export function PlatformDataTable<TData>({
  ...
  const [internalSorting, setInternalSorting] = useState<PlatformDataTableSortState | null>(
    sorting?.defaultValue || null,
  );
  const [internalSelectedIds, setInternalSelectedIds] = useState<Set<string>>(
    () => normalizePlatformDataTableIds(selection?.defaultValue),
  );
  const [internalPagination, setInternalPagination] = useState<PlatformDataTablePaginationState>(
    () => normalizePaginationState(typeof pagination === "object" ? pagination.defaultValue : undefined),
  );
  const [internalSearchValue, setInternalSearchValue] = useState(toolbar?.search?.defaultValue || "");
  const [rowMenu, setRowMenu] = useState<RowMenuState | null>(null);
  const [toolbarMenu, setToolbarMenu] = useState<ToolbarMenuState | null>(null);
  const [rowMenuPosition, setRowMenuPosition] = useState({ left: VIEWPORT_GUTTER, top: VIEWPORT_GUTTER });
  const [toolbarMenuPosition, setToolbarMenuPosition] = useState({ left: VIEWPORT_GUTTER, top: VIEWPORT_GUTTER });

  const data = useMemo(() => Array.from(rows || []), [rows]);
  const sortingControlled = sorting?.value !== undefined;
  const activeSorting = sortingControlled ? sorting?.value || null : internalSorting;
  const selectionControlled = selection?.value !== undefined;
  const selectedIds = useMemo(
    () => selectionControlled ? normalizePlatformDataTableIds(selection?.value) : internalSelectedIds,
    [internalSelectedIds, selection?.value, selectionControlled],
  );
  const searchControlled = toolbar?.search?.value !== undefined;
  const searchValue = searchControlled ? toolbar?.search?.value || "" : internalSearchValue;
  ...
  const hasActions = Boolean(getRowActions || renderRowMenu || onRowActionTrigger);

  useLayoutEffect(() => {
    const element = rootRef.current;
    if (!element) return;
    const updateWidth = () => setContainerWidth(element.getBoundingClientRect().width || Number.POSITIVE_INFINITY);
    updateWidth();
  ...
  const visibleColumns = useMemo(
    () => columns.filter((column) => !column.hideBelow || containerWidth >= column.hideBelow),
  ...
  const tanstackColumns = useMemo<ColumnDef<TData>[]>(() => columns.map((column) => {
    ...
    const definition: ColumnDef<TData> = {
      ...
      cell: (cellContext) => {
        ...
        return column.cell
          ? column.cell({ row: cellContext.row.original, rowId, rowIndex: cellContext.row.index, value })
  ...
  const sortingState = useMemo(() => toSortingState(activeSorting), [activeSorting]);
  const rowSelectionState = useMemo(() => toRowSelectionState(selectedIds), [selectedIds]);
  const paginationState = useMemo<PaginationState>(() => ({
    pageIndex: activePagination.pageIndex,
    pageSize: activePagination.pageSize,
  }), [activePagination.pageIndex, activePagination.pageSize]);
  const manualPageCount = paginationEnabled && paginationConfig?.manual
    ? Math.max(1, Math.ceil(Math.max(0, Number(paginationConfig.totalCount ?? data.length)) / activePagination.pageSize))
    : undefined;
  ...
  const table = useReactTable({
    ...
    enableRowSelection: (row) => selection?.isRowSelectable ? selection.isRowSelectable(row.original) : true,
    globalFilterFn: (row, _columnId, filterValue) => {
      const normalizedFilter = String(filterValue || "").trim().toLocaleLowerCase();
      if (!normalizedFilter) return true;
      const source = toolbar?.search?.getSearchText
        ? toolbar.search.getSearchText(row.original)
        : visibleColumns.map((column) => {
            if (typeof column.accessor === "function") return column.accessor(row.original);
            if (column.accessor != null) return row.original[column.accessor];
            return "";
          }).join(" ");
      return String(source || "").toLocaleLowerCase().includes(normalizedFilter);
    },
  });

  const commitSorting = useCallback((next: PlatformDataTableSortState | null) => {
    if (!sortingControlled) setInternalSorting(next);
    sorting?.onChange?.(next);
  }, [sorting?.onChange, sortingControlled]);

  const commitPagination = useCallback((next: PlatformDataTablePaginationState) => {
  ...
  const sortColumn = useCallback((column: PlatformDataTableColumn<TData>) => {
    if (!column.sortable) return;
    commitSorting(getNextPlatformDataTableSort(activeSorting, column.id, Boolean(column.sortDescFirst)));
    resetPagination();
  ...
  const resolveRowActions = useCallback((row: TData, rowId: string) => {
    const isSelected = selectedIds.has(rowId);
    const targetRows = isSelected && selectedRows.length > 1 ? selectedRows : [row];
  ...
  const commitSelection = useCallback((next: Set<string>, reason: "row" | "visible" | "programmatic") => {
  ...
  const pageCount = Math.max(1, Math.ceil(totalRowCount / activePagination.pageSize));
  const resolvedPageIndex = Math.min(activePagination.pageIndex, pageCount - 1);
  const pageRangeStart = totalRowCount ? resolvedPageIndex * activePagination.pageSize + 1 : 0;
  ...
  ].map((option) => Math.max(1, Math.floor(Number(option) || DEFAULT_PAGE_SIZE))))).sort((left, right) => left - right), [
    activePagination.pageSize,
    paginationConfig?.pageSizeOptions,
  ]);

  useEffect(() => {
    if (!paginationEnabled || resolvedPageIndex === activePagination.pageIndex) {
  ...
  }, [activePagination, commitPagination, paginationEnabled, resolvedPageIndex, totalRowCount]);
  ...
  const toggleRowSelection = useCallback((rowId: string) => {
    commitSelection(togglePlatformDataTableSelection(selectedIds, rowId), "row");
  }, [commitSelection, selectedIds]);

  const toggleVisibleSelection = useCallback(() => {
    commitSelection(
      togglePlatformDataTableVisibleSelection(selectedIds, visibleSelectableIds, !allVisibleSelected),
      "visible",
    );
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      if (target && (rowMenuRef.current?.contains(target) || toolbarMenuRef.current?.contains(target))) return;
      closeMenus();
    };
  ...
  useLayoutEffect(() => {
    ...
    setRowMenuPosition(getFloatingPosition(rowMenu.anchor, rect.width || DEFAULT_ACTION_MENU_WIDTH, rect.height || 40));
  }, [rowMenu]);

  useLayoutEffect(() => {
    if (!toolbarMenu || !toolbarMenuRef.current) return;
    const rect = toolbarMenuRef.current.getBoundingClientRect();
    setToolbarMenuPosition(getFloatingPosition(toolbarMenu.anchor, rect.width || DEFAULT_ACTION_MENU_WIDTH, rect.height || 40));
  }, [toolbarMenu]);

  const openRowMenu = useCallback((event: ReactMouseEvent, rowId: string, contextMenu = false) => {
    event.preventDefault();
    event.stopPropagation();
    const row = data.find((candidate) => getRowId(candidate) === rowId);
    if (!row) return;
    const actions = resolveRowActions(row, rowId).filter((action) => !action.hidden);
    if (!actions.length && !renderRowMenu) return;
    const rect = contextMenu ? null : (event.currentTarget as HTMLElement).getBoundingClientRect();
    setToolbarMenu(null);
    setRowMenu({
      rowId,
      anchor: contextMenu
        ? { x: event.clientX, y: event.clientY, alignRight: false }
        : { x: rect?.right || event.clientX, y: (rect?.bottom || event.clientY) + 6, alignRight: true },
    });
  ...
  const openToolbarMenu = useCallback((event: ReactMouseEvent) => {
    ...
    setRowMenu(null);
    setToolbarMenu((current) => current
      ? null
  ...
  const gridTemplateColumns = useMemo(() => [
    ...
    hasActions ? "28px" : "",
  ].filter(Boolean).join(" "), [hasActions, hasSelection, visibleColumns]);

  const rootStyle = {
    ...style,
    "--platform-data-table-columns": gridTemplateColumns,
    "--platform-data-table-row-min-height": `${Math.max(36, rowMinHeight)}px`,
    "--platform-data-table-sticky-top": typeof stickyTop === "number" ? `${stickyTop}px` : stickyTop,
  } as CSSProperties;
  ...
  const renderToolbar = () => {
    ...
    return createElement("div", {
        className: joinClassNames("platform-data-table__toolbar", toolbar.className),
      },
      toolbar.title
      ...
      filters.length
        ? createElement("button", {
            type: "button",
            className: joinClassNames("platform-data-table__toolbar-button", "is-icon-only", toolbarMenu && "is-open"),
            title: "Filter",
            "aria-label": "Filter",
      ...
      createElement("div", { className: "platform-data-table__toolbar-controls" },
        ...
        toolbar.search
          ? createElement(PlatformSearch, {
              ...
              "aria-label": toolbar.search.ariaLabel || toolbar.search.placeholder || "Search table",
              onChange: (event) => updateSearch(event.currentTarget.value),
            })
          : null,
        viewOptions.length
          ? createElement("div", { className: "platform-data-table__view-switch", role: "group", "aria-label": "View" },
              viewOptions.map((option) => {
                ...
                return createElement("button", {
                  ...
                  renderIcon(option.icon || fallbackIcon, "platform-data-table__view-icon", 14),
                );
              }),
            )
          : null,
        toolbar.trailing
          ? createElement("div", { className: "platform-data-table__toolbar-trailing" }, toolbar.trailing)
          : null,
        toolbar.primaryAction
          ? createElement(PlatformPrimaryButton, {
              size: "small",
              type: "button",
              className: joinClassNames("platform-data-table__primary-action", toolbar.primaryAction.className),
              disabled: toolbar.primaryAction.disabled,
              onClick: toolbar.primaryAction.onClick,
              "aria-label": toolbar.primaryAction.ariaLabel || toolbar.primaryAction.label,
            },
            renderIcon(toolbar.primaryAction.icon || Plus, "platform-data-table__primary-action-icon", 15),
  ...
  const renderPagination = () => {
    ...
    return createElement("nav", {
      ...
      createElement("label", { className: "platform-data-table__page-size" },
        createElement("span", { className: "platform-data-table__page-size-label" }, "Rows per page:"),
        createElement("span", { className: "platform-data-table__page-size-control" },
          createElement("select", {
              value: String(activePagination.pageSize),
              disabled: loading,
              "aria-label": "Rows per page",
              onChange: (event) => commitPagination({
                pageIndex: 0,
                pageSize: Number((event.currentTarget as HTMLSelectElement).value),
              }),
            },
            pageSizeOptions.map((pageSize) => createElement("option", {
              key: pageSize,
      ...
      createElement("div", { className: "platform-data-table__page-actions" },
        createElement("button", {
            type: "button",
            className: "platform-data-table__page-button",
            disabled: previousDisabled,
            "aria-label": "Previous page",
            onClick: () => commitPagination({
              ...activePagination,
              pageIndex: Math.max(0, resolvedPageIndex - 1),
            }),
          },
          createElement(ChevronLeft, { width: 17, height: 17, strokeWidth: 1.9, "aria-hidden": true }),
        ),
        createElement("button", {
            type: "button",
            className: "platform-data-table__page-button",
          ...
          createElement(ChevronRight, { width: 17, height: 17, strokeWidth: 1.9, "aria-hidden": true }),
        ),
      ),
    );
  };

  const renderHeader = () => createElement("div", { className: "platform-data-table__header-group", role: "rowgroup" },
    createElement("div", {
      ...
      hasSelection
        ? createElement("div", { className: "platform-data-table__header-cell is-selection", role: "columnheader" },
      ...
      visibleColumns.map((column) => {
        ...
        return createElement("div", {
          ...
          column.sortable
            ? createElement("span", { className: "platform-data-table__sortable-header" },
                createElement("span", { className: "platform-data-table__header-label" }, column.header),
                createElement("button", {
                    type: "button",
      ...
      hasActions
        ? createElement("div", { className: "platform-data-table__header-cell is-actions", role: "columnheader", "aria-label": "Actions" })
        : null,
    ),
  );
  ...
  const renderBody = () => {
    ...
    if (loading) stateContent = createElement("div", { className: "platform-data-table__state is-loading", role: "status" },
    ...
    return createElement("div", { className: "platform-data-table__body", role: "rowgroup" },
      renderedRows.map((tableRow, renderedIndex) => {
        ...
        const actions = resolveRowActions(row, rowId).filter((action) => !action.hidden);
        const menuAvailable = Boolean(actions.length || renderRowMenu || onRowActionTrigger);
        const externalActionOpen = Boolean(isRowActionOpen?.(row));
        const rowCells = new Map(tableRow.getVisibleCells().map((cell) => [cell.column.id, cell]));
        const handleActivate = () => {
          if (!disabled) onRowActivate?.(row);
        };
        const handleRowKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
          if (!onRowActivate || disabled || event.target !== event.currentTarget) return;
        ...
        const rowElement = createElement("div", {
            className: joinClassNames(
              ...
              (rowMenu?.rowId === rowId || externalActionOpen) && "is-menu-open",
              getRowClassName?.(row),
            ),
            role: "row",
            "aria-rowindex": paginationEnabled
              ? resolvedPageIndex * activePagination.pageSize + renderedIndex + 2
              : renderedIndex + 2,
            tabIndex: onRowActivate && !disabled ? 0 : undefined,
            "aria-selected": hasSelection ? (selected ? "true" : "false") : undefined,
            "aria-disabled": disabled ? "true" : undefined,
            "aria-label": getRowAriaLabel?.(row),
            style: { gridTemplateColumns },
            onClick: onRowActivate ? handleActivate : undefined,
            onKeyDown: handleRowKeyDown,
            onPointerEnter: onRowPointerEnter ? () => onRowPointerEnter(row) : undefined,
            onFocus: onRowFocus ? () => onRowFocus(row) : undefined,
            onContextMenu: (event) => {
              onRowContextMenu?.(event, row);
              if (!event.defaultPrevented && menuAvailable) openRowMenu(event, rowId, true);
            },
          },
          hasSelection
            ? createElement("div", { className: "platform-data-table__cell is-selection", role: "cell" },
                renderSelectionControl(
                  selected,
                  false,
                  selection?.ariaLabel?.(row) || `${selected ? "Deselect" : "Select"} row`,
          ...
          visibleColumns.map((column) => {
            ...
            const customClassName = typeof column.className === "function" ? column.className(row) : column.className;
            return createElement("div", {
                key: column.id,
                className: joinClassNames(
          ...
          hasActions
            ? createElement("div", { className: "platform-data-table__cell is-actions", role: "cell" },
                menuAvailable
                  ? createElement("button", {
                      type: "button",
                      className: joinClassNames("platform-data-table__action-button", (rowMenu?.rowId === rowId || externalActionOpen) && "is-open"),
                      "aria-label": `Open actions${getRowAriaLabel?.(row) ? ` for ${getRowAriaLabel(row)}` : ""}`,
                      "aria-haspopup": "menu",
                      "aria-expanded": rowMenu?.rowId === rowId || externalActionOpen ? "true" : "false",
                      onClick: (event) => {
                    ...
                    }, createElement(EllipsisVertical, { width: 16, height: 16, strokeWidth: 1.8 }))
                  : null,
              )
            : null,
        );
        const expandedContent = isRowExpanded?.(row) && renderExpandedRow
          ? renderExpandedRow({
              row,
              rowId,
          ...
          : null;
        if (expandedContent == null) return createElement("div", { key: rowId, className: "platform-data-table__row-group" }, rowElement);
        return createElement("div", { key: rowId, className: "platform-data-table__row-group is-expanded" },
          rowElement,
          createElement("div", { className: "platform-data-table__expanded-row", role: "row" },
            createElement("div", {
              className: "platform-data-table__expanded-cell",
              role: "cell",
              "aria-colspan": visibleColumns.length + (hasSelection ? 1 : 0) + (hasActions ? 1 : 0),
            }, expandedContent),
          ),
        );
      }),
  ...
  const renderRowActionMenu = () => {
    if (!rowMenu || typeof document === "undefined" || !document.body) return null;
    const row = data.find((candidate) => getRowId(candidate) === rowMenu.rowId);
    if (!row) return null;
    const actions = resolveRowActions(row, rowMenu.rowId).filter((action) => !action.hidden);
    const useSelectedRows = selectedIds.has(rowMenu.rowId) && selectedRows.length > 1;
    const targetRows = useSelectedRows ? selectedRows : [row];
    const targetIds = new Set(targetRows.map((targetRow) => getRowId(targetRow)));
    ...
    const content = customContent || actions.map((action: PlatformDataTableAction<TData>) => createElement("button", {
        key: action.id,
        type: "button",
        role: "menuitem",
        className: joinClassNames(
        ...
        onClick: () => {
          ...
          if (result && typeof result.catch === "function") {
            result.catch((actionError) => console.error("[PlatformDataTable] Row action failed", actionError));
          }
        },
      },
      createElement("span", { className: "platform-data-table__menu-icon" }, renderIcon(action.icon, "", 15)),
      createElement("span", { className: "platform-data-table__menu-label" }, action.label),
    ));
    return createPortal(
      createElement(PlatformPopupSurface, {
          ref: rowMenuRef,
  ...
  const renderToolbarFloatingMenu = () => {
    if (!toolbarMenu || typeof document === "undefined" || !document.body) return null;
    const content = (toolbar?.filters || []).flatMap((filterConfig, filterIndex) => [
        (toolbar?.filters?.length || 0) > 1
          ? createElement("div", { key: `${filterConfig.id}:label`, className: "platform-data-table__menu-heading" }, filterConfig.label)
          : null,
        ...filterConfig.options.map((option) => {
          ...
          return createElement("button", {
            ...
            createElement("span", { className: "platform-data-table__menu-icon" }, active ? createElement(Check, { width: 14, height: 14 }) : null),
            createElement("span", { className: "platform-data-table__menu-copy" },
              createElement("span", { className: "platform-data-table__menu-label" }, option.label),
              option.description ? createElement("span", { className: "platform-data-table__menu-description" }, option.description) : null,
            ),
          );
        }),
        filterIndex < (toolbar?.filters?.length || 0) - 1
          ? createElement("div", { key: `${filterConfig.id}:separator`, className: "platform-data-table__menu-separator" })
          : null,
      ]);
    return createPortal(
      createElement(PlatformPopupSurface, {
          ...
          animation: "down-in",
          role: "menu",
          style: { left: toolbarMenuPosition.left, top: toolbarMenuPosition.top },
          onClick: (event) => event.stopPropagation(),
        }, content),
      document.body,
    );
