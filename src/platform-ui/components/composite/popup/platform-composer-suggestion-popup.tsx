...
export type PlatformComposerSuggestionPopupPlacement = "top" | "bottom";

export interface PlatformComposerSuggestionPopupProps {
  ariaLabel: string;
  children: ReactNode;
  className?: string;
...
/** Shared popup used by slash commands, connector mentions, and comments. */
export function PlatformComposerSuggestionPopup({
  ariaLabel,
  children,
  className = "",
  emptyState,
  ...
  const portalStyle = usePlatformAnchoredPopupStyle({
  ...
  useEffect(() => {
    if (!keyboardNavigation || !surfaceRef.current || !Number.isFinite(activeIndex)) return;
    ...
    if (!activeButton || !scrollContainer || !scrollContainer.contains(activeButton)) return;
  ...
  return portal && anchorRef
    ? renderPlatformAnchoredPopupPortal(content, portalStyle)
    : content;
}
