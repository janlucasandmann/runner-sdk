...
The public `index.mjs` is the only import surface required by `examples/demo-server.mjs`. The demo remains responsible for composing shared services (Runner Chat, projects, environments, agents, teams, billing, and navigation history) and passes those capabilities into Imagine explicitly.
