...
/**
 * Maps the browser-facing platform API to the canonical Thread v2 backend.
 * Query parameters are forwarded by the proxy transport, not encoded here.
 */
export function matchThreadProxyRoute(rawMethod, pathname) {
    const method = String(rawMethod || "GET").toUpperCase();
    if (!String(pathname || "").startsWith(THREAD_V2_PREFIX))
        return null;
...
export function wantsThreadEventStream(req, url) {
